...
const getAllPurchaseOrders = async (req, res) => {
  try {
    const purchaseOrders = await InvPo.find({ cafe: null, type: "PO" })
      .populate("vendor_id")
    ...
    return res.status(200).json({
      status: true,
      message: "Superadmin PO fetched successfully",
      data: purchaseOrders,
    });
  } catch (err) {
    console.error("Error fetching superadmin PO:", err);
    return res.status(500).json({
      status: false,
      message: "Error fetching superadmin PO",
      error: err.message,
    });
  }
};
...
const createPurchaseReceive = async (req, res) => {
  try {
    const { vendor_id, po_id, received_date, description, items } = req.body;
    let qtyToReceive = 0;
...
const getAllPurchaseReceives = async (req, res) => {
  try {
    const purchaseReceives = await InvPo.find({ cafe: null, type: "PR" })
...
const createPurchaseBill = async (req, res) => {
  try {
    ...
    if (req.body.refer_id) {
      await InvPo.findByIdAndUpdate(req.body.refer_id, { status: "Billed" });
    }
...
const getAllPurchaseBills = async (req, res) => {
  try {
    const purchaseBills = await InvPo.find({ cafe: null, type: "PB" })
      .populate("vendor_id")
      .populate("customer_id")
      .populate({
        path: "items",
